...
<![endif]-->
	...
	<div class="am-cf admin-main">
		...
		<!-- sidebar end -->
		<!-- content start -->
		<form action="/restaurant/TableCtrl?action=settledone&id=<%=request.getAttribute("id").toString()%>" method="post" class="am-form">
			<fieldset>
				<legend>
					</br>餐桌<%=request.getAttribute("id")%>结账
				</legend>
	
				<div class="am-form-group">
					<label for="doc-ipt-3" class="am-form-label">人数&nbsp</label>
						<span><% 
							int tid=Integer.valueOf(request.getAttribute("id").toString());
							TableVo tv=new TableModel().selectOneTable(tid); 
							out.print(tv.getSize());
						%></span>
				</div>
				<div class="am-form-group">
					<label for="doc-ipt-3" class="am-form-label">消费</label>
						<span><% 
							out.print(tv.getSize()*50);
						%></span>
				</div>
				<div class="am-form-group">
					<label for="doc-ipt-3" class="am-form-label">折扣</label>
						<span><% 
							OrderVo ov=new OrderModel().selectOrderByTid(tid);
							out.print(ov.getDiscount());
						%></span>
				</div>
				<div class="am-form-group">
					<label for="doc-ipt-3" class="am-form-label">收款</label>
						<span><% 
						if(tv.isHasone()==true){
							out.print(ov.getCost());
						}else{
							out.print("0.0");
						}
						%></span>
				...
				<p>
					<button type="submit" class="am-btn am-btn-success am-btn-sm">
						<span class="am-icon-shopping-cart "> 确认</span>
					</button>
					<a class="am-btn am-btn-warning am-btn-sm"
							href="index.jsp">
							<span class="am-icon-mail-reply "> 取消</span>
					</a>
				</p>
			</fieldset>
